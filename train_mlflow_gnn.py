...
MODEL_DIR = "artifacts"
MODEL_PATH = os.path.join(MODEL_DIR, "oa_deepfplearn_gnn.keras")
...
EXPERIMENT_NAME = "OA-DeepFPLearn-GNN-PFAS"
...
def smiles_to_graph(smiles, max_atoms=MAX_ATOMS):
    ...
    if mol is None:
        return (
            np.zeros((max_atoms, ATOM_FEATURE_DIM), dtype=np.float32),
            np.zeros((max_atoms, max_atoms), dtype=np.float32)
        )
...
def build_oa_deepfplearn_gnn_model(
    fingerprint_dim,
    ontology_dim,
    ...
    g = GraphConv(64, activation="relu", name="gnn_conv_1")([atom_input, adjacency_input])
    g = GraphConv(128, activation="relu", name="gnn_conv_2")([g, adjacency_input])
    g = GraphConv(128, activation="relu", name="gnn_conv_3")([g, adjacency_input])
    g = tf.keras.layers.GlobalAveragePooling1D(name="gnn_global_pooling")(g)
    g = tf.keras.layers.Dense(64, activation="relu", name="gnn_molecular_embedding")(g)
    ...
    fused = tf.keras.layers.Concatenate(name="fp_gnn_ontology_fusion")(
        [latent_fp, g, onto]
    )
...
def transfer_encoder_weights(autoencoder, classifier):
    for auto_layer, clf_layer in zip(autoencoder.layers[1:5], classifier.layers[4:8]):
        try:
            clf_layer.set_weights(auto_layer.get_weights())
        except Exception:
            pass
...
def generate_explainability_artifacts(model, Xfp_test, Xatoms_test, Xadj_test, Xonto_test, df_test):
    ...
    attention_path = os.path.join(explain_dir, "attention_weights_fp_gnn_ontology.csv")
...
def main():
    ...
    validated_arrays, dataset_stats_path = validate_training_arrays(
        df,
        {"X_fp": X_fp, "X_atoms": X_atoms, "X_adj": X_adj, "X_onto": X_onto, "y": y}
    )
    X_fp = validated_arrays["X_fp"]
    X_atoms = validated_arrays["X_atoms"]
    X_adj = validated_arrays["X_adj"]
    ...
    params = {
        "model_type": "OA-DeepFPLearn-GNN",
        "description": "DeepFPLearn latent fingerprints + molecular GNN + ontology embeddings + attention fusion",
    ...
    with mlflow.start_run(run_name="oa_deepfplearn_gnn_run"):
        ...
        model = build_oa_deepfplearn_gnn_model(
            fingerprint_dim=X_fp.shape[1],
            ontology_dim=X_onto.shape[1]
        )
        ...
        print("\nFinal test metrics:")
        print(metrics)
        print(f"\nSaved OA-DeepFPLearn-GNN model to: {MODEL_PATH}")
