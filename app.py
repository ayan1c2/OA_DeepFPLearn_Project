...
if page == "Platform Overview":
    ...
    if st.button("Analyze"):
        ...
        st.metric("Predicted Toxicity Probability", f"{probability:.3f}")
        ...
        st.subheader("Domain Knowledge Interpretation")

        st.write("### Ontology Knowledge")
        st.info(
            f"The compound belongs to the **{chem_class}** class, which has known associations with toxicological endpoints represented in the ontology graph."
        )

        st.write("### Ontology Reasoning Path")
        ...
        if probability < 0.4:
            interpretation = (
                f"Although **{chem_class}** is associated with elevated toxicological concern in the ontology, the current OA-DeepFPLearn model predicts a relatively low probability that this specific compound exhibits the toxicity endpoint represented in the training data."
            )
            confidence = "Moderate"
        elif probability < 0.7:
            interpretation = (
                "The model predicts moderate evidence of toxicity. Both molecular structure and ontology knowledge suggest further investigation may be appropriate."
            )
            confidence = "Moderate to High"
        else:
            interpretation = (
                "Both the molecular prediction and ontology knowledge indicate substantial toxicological concern."
            )
            confidence = "High"
        ...
        if probability < 0.4:
            recommendation = (
                "Further evaluation may be warranted because the chemical belongs to a class with documented toxicological concern despite the low predicted toxicity probability."
            )
        elif probability < 0.7:
            recommendation = (
                "Prioritize additional screening, literature review, and supporting evidence collection."
            )
        else:
            recommendation = (
                "Prioritize regulatory review, monitoring, and advanced toxicological assessment."
            )
        ...
        if actions:
            st.write("### Ontology Recommended Actions")
            for a in actions:
                st.write("•", a)
