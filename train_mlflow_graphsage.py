...
MODEL_DIR = "artifacts"
MODEL_PATH = os.path.join(MODEL_DIR, "oa_deepfplearn_graphsage.keras")
AUTOENCODER_PATH = os.path.join(MODEL_DIR, "fingerprint_autoencoder_graphsage.keras")
ENCODER_PATH = os.path.join(MODEL_DIR, "fingerprint_encoder_graphsage.keras")
EXPERIMENT_NAME = "OA-DeepFPLearn-GraphSAGE-PFAS"
...
class GraphSAGELayer(tf.keras.layers.Layer):
    def __init__(self, units, activation="relu", dropout_rate=0.0, l2_reg=0.0, **kwargs):
        ...
        self.self_dense = None
        self.neigh_dense = None
        self.norm = tf.keras.layers.LayerNormalization()

    def build(self, input_shape):
        feature_dim = int(input_shape[0][-1])
        reg = tf.keras.regularizers.l2(self.l2_reg) if self.l2_reg else None
        self.self_dense = tf.keras.layers.Dense(self.units, kernel_regularizer=reg, use_bias=False)
        self.neigh_dense = tf.keras.layers.Dense(self.units, kernel_regularizer=reg, use_bias=True)
    ...
    def call(self, inputs, training=False):
        ...
        neigh = tf.matmul(adj, x) / degree
        h = self.self_dense(x) + self.neigh_dense(neigh)
        h = self.norm(h)
...
@tf.keras.utils.register_keras_serializable(package="OADeepFPLearn")
class MaskedGlobalPooling(tf.keras.layers.Layer):
    def __init__(self, mode="mean_max", **kwargs):
        super().__init__(**kwargs)
        self.mode = mode
    ...
    def call(self, inputs):
        ...
        x_masked = x * mask_exp
        denom = tf.maximum(tf.reduce_sum(mask_exp, axis=1), 1.0)
        mean_pool = tf.reduce_sum(x_masked, axis=1) / denom
        max_pool = tf.reduce_max(tf.where(mask_exp > 0, x, tf.fill(tf.shape(x), -1e9)), axis=1)
        max_pool = tf.where(tf.math.is_finite(max_pool), max_pool, tf.zeros_like(max_pool))
        if self.mode == "mean":
            return mean_pool
        if self.mode == "max":
            return max_pool
...
def build_fingerprint_autoencoder(fingerprint_dim, latent_dim=LATENT_FP_DIM):
    ...
    x = tf.keras.layers.Dense(1024, activation="relu", kernel_regularizer=tf.keras.regularizers.l2(L2_REG))(inp)
    ...
    autoencoder = tf.keras.Model(inp, out, name="DeepFPLearn_Fingerprint_Autoencoder")
    encoder = tf.keras.Model(inp, latent, name="DeepFPLearn_Fingerprint_Encoder")
    autoencoder.compile(optimizer=tf.keras.optimizers.Adam(LEARNING_RATE), loss="binary_crossentropy", metrics=["mse"])
    return autoencoder, encoder


def build_graphsage_model(fingerprint_dim, ontology_dim):
    ...
    fp = tf.keras.layers.Dense(1024, activation="relu", kernel_regularizer=tf.keras.regularizers.l2(L2_REG))(fp_input)
    fp = tf.keras.layers.BatchNormalization()(fp)
    fp = tf.keras.layers.Dropout(DROPOUT_RATE)(fp)
    fp = tf.keras.layers.Dense(512, activation="relu", kernel_regularizer=tf.keras.regularizers.l2(L2_REG))(fp)
    fp = tf.keras.layers.Dense(256, activation="relu", kernel_regularizer=tf.keras.regularizers.l2(L2_REG))(fp)
    latent_fp = tf.keras.layers.Dense(LATENT_FP_DIM, activation="relu", name="deepfplearn_latent_fingerprint")(fp)

    g = GraphSAGELayer(96, dropout_rate=DROPOUT_RATE, l2_reg=L2_REG, name="graphsage_layer_1")([atom_input, adjacency_input])
    g2 = GraphSAGELayer(128, dropout_rate=DROPOUT_RATE, l2_reg=L2_REG, name="graphsage_layer_2")([g, adjacency_input])
    g3 = GraphSAGELayer(128, dropout_rate=DROPOUT_RATE, l2_reg=L2_REG, name="graphsage_layer_3")([g2, adjacency_input])
    graph_embedding = MaskedGlobalPooling(name="graphsage_masked_pooling")([g3, atom_mask_input])
    graph_embedding = tf.keras.layers.Dense(128, activation="relu", name="graphsage_molecular_embedding")(graph_embedding)
    ...
    fused = tf.keras.layers.Concatenate(name="fp_graphsage_ontology_fusion")([latent_fp, graph_embedding, onto])
    ...
    output = tf.keras.layers.Dense(1, activation="sigmoid", name="toxicity_probability")(x)

    model = tf.keras.Model(
        inputs=[fp_input, atom_input, adjacency_input, atom_mask_input, onto_input],
        outputs=output,
        name="OA_DeepFPLearn_GraphSAGE",
    )
    model.compile(
...
def prepare_dataset():
    ...
    return df, *arrays, graph


def validate_arrays(df, arrays):
    os.makedirs(MODEL_DIR, exist_ok=True)
    fixed = {}
    for name, arr in arrays.items():
        fixed[name] = np.nan_to_num(arr, nan=0.0, posinf=0.0, neginf=0.0)
    stats = {
        "dataset_size": int(len(df)),
        "class_distribution": {str(k): int(v) for k, v in df["toxicity"].value_counts().to_dict().items()},
        "feature_shapes": {k: list(v.shape) for k, v in fixed.items()},
    }
    stats_path = os.path.join(MODEL_DIR, "dataset_statistics_graphsage.json")
    with open(stats_path, "w", encoding="utf-8") as f:
        json.dump(stats, f, indent=2)
    return fixed, stats_path


def generate_visualizations(model, Xfp_test, Xatoms_test, Xadj_test, Xmasks_test, Xonto_test, df_test, y_test, probs):
    import matplotlib.pyplot as plt
    out_dir = os.path.join(MODEL_DIR, "graphsage_visualizations")
    ...
    attention_path = os.path.join(out_dir, "graphsage_fusion_attention_weights.csv")
    ...
    pred_path = os.path.join(out_dir, "graphsage_predictions.csv")
    ...
    plt.title("GraphSAGE-DeepFPLearn Prediction Probability Distribution")
    hist_path = os.path.join(out_dir, "graphsage_probability_histogram.png")
    plt.tight_layout()
    plt.savefig(hist_path, dpi=300)
    plt.close()

    branch_names = ["latent_fingerprint", "graphsage_embedding", "ontology_embedding"]
    splits = [LATENT_FP_DIM, 128, 64]
    branch_means = []
    start = 0
    for size in splits:
    ...
    plt.title("GraphSAGE-DeepFPLearn Branch Attention")
    branch_path = os.path.join(out_dir, "graphsage_branch_attention.png")
    plt.tight_layout()
...
def main():
    ...
    arrays, dataset_stats_path = validate_arrays(df, {"X_fp": X_fp, "X_atoms": X_atoms, "X_adj": X_adj, "X_masks": X_masks, "X_onto": X_onto, "y": y})
    ...
    Xfp_train, Xfp_test, Xatoms_train, Xatoms_test, Xadj_train, Xadj_test, Xmasks_train, Xmasks_test, Xonto_train, Xonto_test, y_train, y_test, df_train, df_test = train_test_split(
        X_fp, X_atoms, X_adj, X_masks, X_onto, y, df, test_size=0.25, random_state=RANDOM_STATE, stratify=stratify
    )

    params = {
        "model_type": "OA-DeepFPLearn-GraphSAGE",
        "fingerprint_dim": int(X_fp.shape[1]),
        "ontology_dim": int(X_onto.shape[1]),
        "max_atoms": MAX_ATOMS,
    ...
    with mlflow.start_run(run_name="oa_deepfplearn_graphsage_run"):
        mlflow.log_params(params)
        autoencoder, encoder = build_fingerprint_autoencoder(X_fp.shape[1], LATENT_FP_DIM)
        ...
        model = build_graphsage_model(X_fp.shape[1], X_onto.shape[1])
        class_weight = None
        ...
        model.fit({
            ...
            "atom_mask_input": Xmasks_train,
            "ontology_input": Xonto_train,
        }, y_train, validation_split=0.2, epochs=CLASSIFIER_EPOCHS, batch_size=BATCH_SIZE, class_weight=class_weight,
            callbacks=[
                tf.keras.callbacks.EarlyStopping(monitor="val_auc", patience=15, mode="max", restore_best_weights=True),
                tf.keras.callbacks.ReduceLROnPlateau(monitor="val_auc", factor=0.5, patience=6, mode="max", min_lr=1e-6),
            ], verbose=1)
        ...
        meta_path = os.path.join(MODEL_DIR, "graphsage_model_metadata.json")
        with open(meta_path, "w", encoding="utf-8") as f:
            json.dump({"model_name": "OA-DeepFPLearn-GraphSAGE", "metrics": metrics, "params": params}, f, indent=2)
        ontology_path = os.path.join(MODEL_DIR, "graphsage_ontology_edges.json")
        with open(ontology_path, "w", encoding="utf-8") as f:
            json.dump(list(graph.edges()), f, indent=2)

        for artifact in [MODEL_PATH, AUTOENCODER_PATH, ENCODER_PATH, meta_path, ontology_path, dataset_stats_path]:
            mlflow.log_artifact(artifact)
        for artifact in generate_visualizations(model, Xfp_test, Xatoms_test, Xadj_test, Xmasks_test, Xonto_test, df_test, y_test, probs):
            mlflow.log_artifact(artifact)
